...
LAN User -> Open WebUI (:3000) -> vLLM Model API
                              -> Tool Calls on the same host
                                 ...
                                 -> Suno 작사·스타일 프롬프트 도구
                                    -> http://127.0.0.1:8005 Suno Lyrics/Style Generator
```
...
- `suno-service`: Suno V5/V5.5 작사·스타일 프롬프트 생성·개선 도구. vLLM을 백엔드로 사용해 Suno 가이드 룰(5-Part Formula, 구조 태그 화이트리스트, BPM 표 등)을 적용한 가사·Style 필드 프롬프트를 만들고 다듬음
...
- `POST /web/fetch`
- `POST /suno/lyrics/generate`
- `POST /suno/lyrics/improve`
- `POST /suno/style/generate`
- `POST /suno/style/improve`
...
```bash
bash scripts/start_suno_service.sh
```
...
Suno generator    -> http://127.0.0.1:8005
```
...
- `open-webui/openwebui-suno-tools.json`
