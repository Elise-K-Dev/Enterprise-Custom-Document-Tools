...
- `Python 문서 검색`: 특정 문서, 업무보고 원문, 날짜별 작업 이력, 기존 기록을 찾을 때만 사용할 것
- `Rust 구매 품의서 생성`: 재고, 품목, 현재고, 부족수량, 단가, 구매 우선순위, 구매 품의서 생성에만 사용할 것
- `PDF 보고서 생성기`: Markdown 기반 수리 완료 보고서, 정비 계획, 업무 보고서, 회의록, 일반 요약 보고서, 분석 결과를 PDF 파일로 만들 때 사용할 것
