...
const DATE_PARSE_FORMATS: [&str; 5] =
    ["%Y-%m-%d", "%Y/%m/%d", "%m/%d/%Y", "%-m/%-d/%Y", "%Y.%m.%d"];
...
#[derive(Debug, Clone)]
pub struct LegacyBatchRunSummary {
    pub workdir: PathBuf,
    pub output_dir: PathBuf,
    pub generated_count: usize,
    pub generated_files: Vec<String>,
    pub snapshot_json_path: Option<String>,
    pub batch_report_path: Option<String>,
...
struct RowRecord {
    ...
    header_norm_index: Arc<HashMap<String, usize>>,
...
fn main() -> Result<()> {
    ...
    println!(
        "[translator_for_docu_v2] workdir: {}",
...
pub fn run_batch_once_from_workdir(
    workdir: &Path,
    force: bool,
    smoke_enabled: bool,
) -> Result<LegacyBatchRunSummary> {
    init_rayon_pool();
    let workspace = setup_workspace(workdir)?;
    let batch = match detect_ready_batch(&workspace.input_dir)? {
        DetectResult::Ready(batch) => batch,
        DetectResult::MissingKinds(kinds) => {
            bail!("waiting: missing files [{}]", join_doc_kinds(&kinds));
        }
        DetectResult::UnstableFiles(paths) => {
            bail!("waiting: files still copying [{}]", join_paths(&paths));
        }
    };

    let fingerprint = compute_batch_fingerprint(&batch.inbound, &batch.stock, &batch.outbound)?;
    if !force && is_same_as_last_processed(&workspace, &fingerprint)? {
        bail!("waiting: no new batch changes");
    }

    let stamp = Local::now().format("%Y%m%d_%H%M%S").to_string();
    let artifacts = run_batch_pipeline(
        &workspace,
        &batch.inbound,
        &batch.stock,
        &batch.outbound,
    ...
    let report = BatchReport {
        ...
        snapshot_date: Some(artifacts.snapshot_date.clone()),
        ...
        outputs: Some(BatchReportOutputs {
        ...
        converted_parts: Some(artifacts.converted_parts),
        error: None,
    };
    let report_path = write_batch_report(&workspace, &stamp, &report)?;
    let generated_files = artifacts
        .docx_outdir
        .as_ref()
        .map(|dir| list_docx_files(dir, &workspace.root))
        .transpose()?
        .unwrap_or_default();

    Ok(LegacyBatchRunSummary {
        workdir: workspace.root.clone(),
        output_dir: artifacts
            .docx_outdir
            .unwrap_or_else(|| workspace.output.clone()),
        generated_count: generated_files.len(),
        generated_files,
        snapshot_json_path: Some(path_relative_to(&workspace.root, &artifacts.out_json)),
        batch_report_path: Some(path_relative_to(&workspace.root, &report_path)),
        status_note: format!(
            "processed files: {}, {}, {}",
...
impl AppConfig {
    fn from_args() -> Result<Self> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                "--workdir" => {
                    let next = args.get(i + 1).context("--workdir requires a path")?;
...
fn default_workdir() -> PathBuf {
    if let Ok(path) = env::var("DOC_PIPELINE_WORKDIR") {
...
fn setup_workspace(root: &Path) -> Result<Workspace> {
    ...
    seed_input_dir_from_repo(&ws)?;

    Ok(ws)
}

fn seed_input_dir_from_repo(workspace: &Workspace) -> Result<()> {
    let repo_db_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("DB");
    let repo_input_dir = repo_db_root.join("input");

    if workspace.root == repo_db_root || !repo_input_dir.exists() {
        return Ok(());
    }

    copy_missing_tree(&repo_input_dir, &workspace.input_dir)
}

fn copy_missing_tree(source: &Path, destination: &Path) -> Result<()> {
    for entry in
        fs::read_dir(source).with_context(|| format!("read dir failed: {}", source.display()))?
    {
        let entry = entry?;
...
fn process_available_batch(workspace: &Workspace, smoke_enabled: bool) -> Result<ProcessOutcome> {
    ...
    match write_batch_report(workspace, &stamp, &report) {
        Ok(path) => write_log(
            ...
            &format!(
                "batch report saved: {}",
                path_relative_to(&workspace.root, &path)
            ),
        ),
...
fn generate_docx_from_snapshot(
    ...
    let Some(templates) = resolve_docx_templates_by_name(workspace)? else {
        write_log(
            workspace,
            "named docx templates not found; skipping docx generation",
        );
        return Ok((None, 0));
    };
    ...
    if outdir.exists() {
        fs::remove_dir_all(&outdir)
            .with_context(|| format!("clear previous docx outdir failed: {}", outdir.display()))?;
    }
...
fn resolve_docx_templates_by_name(workspace: &Workspace) -> Result<Option<NamedTemplatePackages>> {
    ...
    let (
        over_500k_with_history_pkg,
        over_500k_with_history_name,
        over_500k_without_history_pkg,
    ...
    let under_eq_500k_with_history_path =
        template_dir.join(TEMPLATE_UNDER_EQ_500K_WITH_HISTORY_DOCX);
    let under_eq_500k_without_history_path =
        template_dir.join(TEMPLATE_UNDER_EQ_500K_WITHOUT_HISTORY_DOCX);

    let (
        under_eq_500k_with_history_pkg,
        under_eq_500k_with_history_name,
        under_eq_500k_without_history_pkg,
...
fn build_document_rows(
    ...
    for (part_key, part) in &snapshot.parts {
        ...
        if let Some(out_idx) = part.outbound_row_idx.last() {
            if let Some(row) = snapshot.raw.outbound_rows.get(*out_idx) {
                let equip_name =
                    pick_first_col(&row.header_norm_index, &row.cells, &k_out_equip_name);
                let equip_no = pick_first_col(&row.header_norm_index, &row.cells, &k_out_equip_no);
                let model_fallback =
                    pick_first_col(&row.header_norm_index, &row.cells, &k_out_model_fallback);
                used_where = if equip_name != "기록없음" {
                ...
                usage_reason =
                    pick_first_col(&row.header_norm_index, &row.cells, &k_out_usage_reason);
                replacement_reason = pick_first_col(
                    &row.header_norm_index,
                    &row.cells,
                    &k_out_replacement_reason,
        ...
        if let Some(row) = stock_row {
            if is_missing_doc_value(&used_where) {
                used_where =
                    pick_first_col(&row.header_norm_index, &row.cells, &k_used_where_stock);
            }
            if is_missing_doc_value(&vendor_name) {
                vendor_name = pick_first_col(&row.header_norm_index, &row.cells, &k_vendor);
            }
...
fn collect_replacement_events(snapshot: &Snapshot, part: &PartBlock) -> Vec<ReplacementEvent> {
    let k_date = normalize_keys(&["지급일자", "출고일자", "date"]);
    let k_qty = normalize_keys(&["지급량", "검수량", "qty"]);
    let k_host = normalize_keys(&["장비번호", "호기", "설비번호", "장비명", "주요장비명"]);
    ...
    for idx in &part.outbound_row_idx {
        if let Some(row) = snapshot.raw.outbound_rows.get(*idx) {
            let date_iso = row
                ...
                .clone()
                .or_else(|| {
                    let raw = pick_first_col(&row.header_norm_index, &row.cells, &k_date);
                    normalize_date_to_iso(&raw)
                })
                .unwrap_or_else(|| "0000-00-00".to_string());
            let qty = row.qty.map(|v| format!("{:.0}", v)).unwrap_or_else(|| {
                let raw = pick_first_col(&row.header_norm_index, &row.cells, &k_qty);
                parse_numeric_text(&raw)
                    .map(|n| format!("{:.0}", n))
                    .unwrap_or_else(|| "0".to_string())
            });
            let host = pick_first_col(&row.header_norm_index, &row.cells, &k_host);
...
fn normalize_keys(keys: &[&str]) -> Vec<String> {
    keys.iter().map(|k| normalize_header_key(k)).collect()
}

fn pick_first_col(
    header_norm_index: &HashMap<String, usize>,
    cells: &[String],
    keys: &[String],
) -> String {
    // normalized key index lookup (precomputed once per sheet)
    for nk in keys {
        if let Some(idx) = header_norm_index.get(nk) {
...
fn load_template_package(template: &Path) -> Result<TemplatePackage> {
    let tf = File::open(template)
        .with_context(|| format!("open template failed: {}", template.display()))?;
...
fn write_last_processed_fingerprint(workspace: &Workspace, current: &str) -> Result<()> {
    fs::write(last_batch_marker_path(workspace), current).context("write last batch marker failed")
}
...
fn list_docx_files(root: &Path, base: &Path) -> Result<Vec<String>> {
    ...
    while let Some(dir) = stack.pop() {
        for entry in
            fs::read_dir(&dir).with_context(|| format!("read dir failed: {}", dir.display()))?
        {
            let entry = entry?;
            let path = entry.path();
            if path.is_dir() {
            ...
            let is_docx = path
                ...
                .unwrap_or(false);
            if is_docx {
                files.push(path_relative_to(base, &path));
            }
        }
    }
...
fn read_inbound(path: &Path) -> Result<Vec<RowRecord>> {
    ...
    let header_norm_index = Arc::new(build_normalized_header_index(&header));
    ...
    let out = rows[1..]
        ...
        .filter_map(|row| {
            ...
            let cells = row.iter().map(cell_to_string_raw).collect::<Vec<_>>();
            Some(RowRecord {
                cells,
                header_norm_index: header_norm_index.clone(),
...
fn build_normalized_header_index(header: &[String]) -> HashMap<String, usize> {
    let mut out = HashMap::with_capacity(header.len());
    for (i, h) in header.iter().enumerate() {
        let nk = normalize_header_key(h);
        if nk.is_empty() {
            continue;
        }
        out.entry(nk).or_insert(i);
    }
    out
}
